...
function getOidcClient(config = getOidcConfiguration()): OAuth2Client {
...
export async function createOidcAuthorizationRequest(): Promise<OidcAuthorizationRequest> {
  ...
  const url = getOidcClient(config).createAuthorizationURLWithPKCE(
...
export async function exchangeOidcAuthorizationCode({
  ...
  const config = getOidcConfiguration();
  const metadata = await getOidcMetadata();
  const tokens = await getOidcClient(config).validateAuthorizationCode(
